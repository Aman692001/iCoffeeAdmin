...
func downloadUserFromFirestore(userId: String, email: String, completion: @escaping (_ error: Error?) -> Void) {
    ...
    FirebaseReference(.User).document(userId).getDocument {(snapshot, error) in
        ...
        if snapshot.exists {
            ...
            saveUserLocally(userDictionary: userDictionnaryFrom(user: user) as NSDictionary)
...
func saveUserToFirestore(fUser: FUser) {
    ...
    FirebaseReference(.User).document(fUser.id).setData(userDictionnaryFrom(user: fUser)) { (error) in
...
func userDictionnaryFrom(user: FUser) -> [String : Any] {
